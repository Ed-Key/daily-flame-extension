...
const emailForm = document.getElementById('email-signin-form') as HTMLFormElement;
...
function showSection(section: 'auth' | 'loading' | 'success' | 'error') {
...
function toggleAuthMode(e: Event) {
  ...
  if (isSignUpMode) {
    if (h2) h2.textContent = 'Create an Account';
